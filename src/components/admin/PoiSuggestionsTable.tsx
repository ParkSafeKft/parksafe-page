                    ...
                    <AdminModalBody className="space-y-6">
                        <AdminModalRelations>
                            <AdminRelationCard
                                label="Beküldő"
                                title={reporterName}
                                description={item.user_id || 'Nincs kapcsolt profil'}
                                icon={<User />}
                                onClick={onOpenUser && item.user_id ? () => onOpenUser(item.user_id!) : undefined}
                            />
                            <AdminRelationCard
                                label="Javasolt hely"
                                title={item.name || getTypeLabel(suggestedType)}
                                description={hasValidCoords ? `${latNumber}, ${lonNumber}` : 'Nincs érvényes koordináta'}
                                icon={<MapPin />}
                                action={hasValidCoords ? (
                                    <a
                                        href={`https://www.openstreetmap.org/edit?#map=19/${latNumber}/${lonNumber}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        title="Hely megnyitása az OSM szerkesztőben"
                                    >
                                        <Edit />
