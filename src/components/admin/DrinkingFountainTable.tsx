...
interface DrinkingFountainTableProps {
...
export default function DrinkingFountainTable({
    data,
    onRowClick,
    onEdit,
    onDelete,
        ...
        return (
            <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-zinc-800 rounded-3xl bg-zinc-900/20">
                <div className="w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center mb-4">
                    <Droplet className="w-8 h-8 text-zinc-700" />
                </div>
                <h3 className="text-lg font-semibold text-white">
                    {searchTerm ? 'Nincs találat' : 'Nincsenek ivókutak'}
                </h3>
                <p className="text-zinc-500 max-w-xs text-center mt-2">
                    {searchTerm
                        ? 'Próbálj meg más keresési kifejezést használni.'
                        : 'Kattints az "Új hozzáadása" gombra az első ivókút létrehozásához.'
                    }
                </p>
            </div>
        );
                                ...
                                >
                                    <td className="p-4">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-semibold text-white">{item.name || 'Ivókút'}</span>
                                            {coords && (
                                                <span className="text-[10px] text-zinc-500 flex items-center gap-1">
                                                    <Droplet className="w-3 h-3" /> {coords.lat.toFixed(4)}, {coords.lon.toFixed(4)}
                                                </span>
                                            )}
                                        </div>
                                    </td>
