...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const menuItems = [
    ...
    { id: 'stations', label: 'Javító Állomások', icon: Wrench, section: 'ADATKEZELÉS' },
              ...
              {menuItems
                ...
                .map(item => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-5 h-5 transition-colors ${isActive ? 'text-green-500' : 'text-zinc-500 group-hover:text-zinc-300'}`} />
                      <span className="text-sm font-medium flex-1 text-left">{item.label}</span>
                      {isActive && <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" />}
                    </button>
                  );
                })}
            </div>
      ...
      <div className="p-4 mt-auto space-y-4">
        <div className="p-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-white truncate">Temp</p>
            <p className="text-[11px] text-zinc-500 truncate">kopamc9@gmail.com</p>
          </div>
        </div>

        <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors text-sm font-medium">
